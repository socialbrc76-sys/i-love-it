...
async function uploadImageWithFallback(imagePath) {
    return await withRetry(async () => {
        try {
            return await uploadToUguu(imagePath);
        } catch (error) {
            console.warn(`  ⚠️ Uguu.se 업로드 실패 (${error.message || error}). tmpfiles.org 백업 시도...`);
            return await uploadToTmpFiles(imagePath);
        }
...
async function publishToInstagram(imagePaths, caption, commentText) {
    ...
    for (let i = 0; i < imagePaths.length; i++) {
        const url = await uploadImageWithFallback(imagePaths[i]);
        imageUrls.push(url);
